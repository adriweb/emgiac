...
id="matr_case'+i+'_'+j+'">'+oldval+'</textarea><div class="matrixcell" style="display:none;width:20px" onclick="UI.sheet_onfocus(this);" id="matr_span'+i+'_'+j+'"></div></td>';
  ...
  restoresession: function(chaine,hist,asked,doexec){
	...
	if (p[1]=='1'){ form.angle_mode.checked=true;}
	continue;
      }
      if (p[0]=='sqrt'){
	if (p[1]=='0'){ form.sqrt_mode.checked=false;}
	if (p[1]=='1'){
  ...
  link: function(start){
	...
	s = UI.base_url+"xcasen.html#"+s;
      ...
      if (UI.forum_warn){ UI.forum_warn=false; copy += UI.langue==-1?"alert(\"Le lien de la session a été copié dans le presse-papier, vous pouvez le coller dans la page qui va apparaitre\");":"alert(\"Clipboard contains link to session, you can paste it in the tab that will open\");"; }
  ...
  exec_history: function(){alert(UI.langue==-1?'Historique!':'History!');},
  svg_counter:0,
  ...
  rendercomment:function(t){ // replace \n by <br>, for $$ call caseval on mathml or latex(quote())
	...
	return t;
      ...
      if (ch=='\\' && nxt=='$'){ if (indollar) cas+=nxt; else res +=nxt; continue; }
  ...
  listvars: function(n){ // n=2 (list) or 3 (list+values)
    ...
    if (!Array.isArray(v)){     f.innerHTML='<br>'+s; return;}
    var l=v.length,i,j,cs;
    s="";
    for (i=0;i<l;i+=2){
      var name=v[i];
      var value=v[i+1],nl=value.length;
      nl=Math.floor(nl/10); if (nl>5) nl=5;
